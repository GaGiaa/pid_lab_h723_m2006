...
| M2006 速度环 | ✅ 已实测 | 2026-09-18 确认 | 同上 | 已跑通；PID 参数已精调固化（spd_pid_kp=200, spd_pid_ki=18000） |
| M2006 位置环 | ✅ 已实测 | 2026-09-18 确认 | 同上 | 已跑通；PID 参数已精调固化（pos_pid_kp=3.0, 死区 0.1°, 限速 300rpm） |
...
- 2026-09-19：PID 默认值固化（上板实测整定后写入代码）——pos_pid_kp 1.0→3.0、pos_deadband_deg 0.5→0.1、pos_max_speed_rpm 0→300、spd_pid_kp 30→200、spd_pid_ki 5→18000；motor.h 注释、单测断言、hardware.md 同步；验证：motor 37 断言 / bus 48 断言 / 命名 19 用例 / Keil 重建全通过。子模块 `585d783`，主工程 `a88f380`，均已 push。
